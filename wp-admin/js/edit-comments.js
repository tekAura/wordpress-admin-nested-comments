...
commentReply = {
	...
	show : function(xml) {
		...
		if ($(id).prev().prev().children().children().hasClass('parent_comment') || $(id).prev().prev().children().children().hasClass('child_comment')) {
			nestedcomment = true;
			marginleft = parseInt($(id).prev().prev().children().children().css('margin-left').split('px')[0]) + 50;
		}

		if (nestedcomment) {
			var commentinner = $(id).children();
			var nestedwrap = "<td colspan='42' style='padding: unset !important;'>";
			nestedwrap += "<table class='child_comment widefat' style='margin-left:" + marginleft + "px; border-spacing: unset;'>";
			nestedwrap += "<tbody>";
			nestedwrap += "<tr>";
			nestedwrap += "</tr></tbody></table></td>";
			$(id).empty();
			$(id).append(nestedwrap);
			$(id).children().children().children().children().append(commentinner);
		}		
