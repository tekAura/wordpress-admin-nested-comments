...
<?php
	 ...
	 * @param WP_Comment $item
	 */
	protected function display_children_rows( $item, $level ) {
		global $getting_children;
		$getting_children = TRUE;
		$comment = $item;
		...
		foreach ( $comment_children as $comment_child ) {
			$the_comment_class = wp_get_comment_status( $comment_child );
			if ( ! $the_comment_class ) {
				$the_comment_class = '';
			}
			$the_comment_class = join( ' ', get_comment_class( $the_comment_class, $comment_child, $comment_child->comment_post_ID ) );

			echo "<tr id='comment-$comment_child->comment_ID' class='$the_comment_class'>";
				echo "<td colspan='42' style='padding: unset !important;'>";
					echo "<table class='child_comment widefat' style='margin-left:" . $margin_left . "px; border-spacing: unset;'>";
						echo "<tbody>";
							echo "<tr>";
								$this->single_row_columns( $comment_child );
							echo "</tr>\n";
						echo "</tbody>";
					echo "</table>";
				echo "</td>";
			...
			$this->display_children_rows( $comment_child, $level );
